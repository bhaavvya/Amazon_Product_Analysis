...
product_type_dataset = ["data_kidsboys.csv","data_kidsgirls.csv"]
...
def graph_1(data,kid):
    ...
    chart_type = FigureCanvasTkAgg(plt.gcf(), master=kid)
    chart_type.draw()
    chart_type.get_tk_widget().grid(row=2, column=0, columnspan=4)
    

def graph_2(data,kid):
